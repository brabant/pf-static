...
$(document).ready(function () {
  if (document.querySelector('.js-signup-form')) {
    var flagE = false;
    var flagJ = false; // on tabs click

    $('.js-signup-tab').click(function (e) {
      // change footer color
      $('.js-signup-footer').css('backgroundColor', 'white');
      $('.js-signup-tab').removeClass('is-invalid'); // change role flag
    ...
    $('.js-signup-form').submit(function (event) {
      ...
      var formData = {}; // getting common form values
      ...
      function userLogin() {
        ...
        $.ajax({
          ...
          success: function success(response) {
            ...
            var metrikaParams = {
              myParam: 'Рега в кейсе',
              email: loginData.email
            };

            var metrikaCallback = function metrikaCallback() {
              return console.log('Запрос в Метрику отправлен');
            };

            window.ym(27035592, 'reachGoal', 'RegistrationSuccess', metrikaParams, metrikaCallback()); //window.location.href = "https://pressfeed.ru/all-queries";
    ...
    var passwordIcon = document.querySelector('.js-password-icon');
